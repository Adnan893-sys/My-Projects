...
.def  Smallest_Value  =r1             ;smallestest value in R30 register
.def  location_Smallest =r2         ; index of biggest
.def  Index_Smallest =r3            ; location of the smallestest in memory
...
loop:
	...
	cp temp,Smallest_Value          ;compare that value to the smallest value already
	brge next                       ; if greater than goto else
	mov Smallest_Value,temp         ;store smallest value 
	mov location_Smallest,ZL        ; store its index 
	mov Index_Smallest,counter      ; store its location in register
