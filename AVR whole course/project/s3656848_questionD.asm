...
.def  temp  =r16                        //keep temp
.def  Smallest_Value  =r17             //keep smallest value
.def  Smallest_Value_location =r18    //keep smallest value location
.def  Smallest_Value_Index =r19      // keep smallest value index
.def  increment_counter =r20        //keep counter
...
loop:
    inc increment_counter           ;increment counter
	LPM temp, Z						; Load constant from table in memory pointed to by Z (r31:r30)
	cp temp,Smallest_Value          ;comapre temp value with smallest value
	brge do_nothing                 ; barnch if graeter
	mov Smallest_Value,temp         ;else save temp value in smallest_value register
	mov Smallest_Value_location,ZL   ; also save locotion of this in memory
	mov Smallest_Value_index,increment_counter   ; also count index
