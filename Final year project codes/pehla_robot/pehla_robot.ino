...
ros::Publisher chatter("rfid1", &str_msg);
...
ros::Subscriber<std_msgs::String> sub("robot1", &messageCb );
ros::Subscriber<std_msgs::String> Status("status1", &callback );
...
void forward(){
  ...
  analogWrite(rmotorenable, 90);
}

void left(){
  analogWrite(lmotorenable, 60);
  analogWrite(rmotorenable, 100);
}
void right(){
  analogWrite(lmotorenable, 100);
  analogWrite(rmotorenable, 60);
}
void turn_left(){
